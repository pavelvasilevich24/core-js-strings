...
function removeFirstOccurrences(str, value) {
  const occurIndex = str.indexOf(value);
  if (occurIndex !== -1) {
    return str.slice(0, occurIndex) + str.slice(occurIndex + value.length);
  }
  return str;
}
...
function removeLastOccurrences(str, value) {
  const occurIndex = str.lastIndexOf(value);
  if (occurIndex !== -1) {
    return str.slice(0, occurIndex) + str.slice(occurIndex + value.length);
  }
  return str;
...
function isPalindrome(str) {
  const replaced = str.toLowerCase().replaceAll(/[^a-z]+/g, '');
  const reverted = replaced.split('').reverse().join('');
